            ...
            if not hitsWall:   
                ...
                for i in range (0,len(self.corners)):
                   if(self.corners[i]==nextstate):
                       visited[i]=True
                successornew=(((nextstate,visited),action,1))
                successors.append(successornew)
...
def cornersHeuristic(state, problem):
    ...
    heuristics=[0]
    if (not len(unvisited)==0): 
        for place in unvisited:
            heuristics.append(util.manhattanDistance(current,place))
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    locations = foodGrid.asList()
    heuristic = [0]
    for loc in locations:
        #heuristic.append(util.manhattanDistance(position,loc))
        heuristic.append(mazeDistance(position,loc,problem.startingGameState))
